      ...
      SetSlotsResult: function SetSlotsResult(cards) {
        ...
        for (var i = 0; i < cards.length; i++) {
          ...
          if (this._cfg.symbol[cards[i]]) {
            ...
            res.data = {};
            var pi = [];
            Array.isArray(pawIdxs) && (pi = pawIdxs);
            pi && pi.includes(i + 1) && (res.data.paw = 1);
            var bi = [];
            Array.isArray(bonusIdxs) && (bi = bonusIdxs);
            bi && bi.includes(i + 1) && (res.data.bonus = bonusItems[bonusIdxs.indexOf(i + 1)]);
            reelResults[col] || (reelResults[col] = []);
            reelResults[col].unshift(res);
          }
        }
