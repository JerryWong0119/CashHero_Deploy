    ...
    cc.Class({
      ...
      SetSlotsResult: function SetSlotsResult(cards) {
        ...
        if (spinData.bonusInfo) {
          var loseItems = spinData && spinData.bonusInfo && spinData.bonusInfo.loseItems;
          Array.isArray(loseItems) || (loseItems = []);
          var bonusItems = loseItems.concat(spinData.bonusInfo.winItems);
          Array.isArray(bonusItems) || (bonusItems = []);
          var _iterator = _createForOfIteratorHelper(bonusItems), _step;
          try {
            for (_iterator.s(); !(_step = _iterator.n()).done; ) {
              var item = _step.value;
              var idxInfo = this.transferIdx(item.idx);
              if (item.idx && idxInfo && idxInfo.col && idxInfo.row) {
                var data = reelResults[idxInfo.col][idxInfo.row].data;
                item.coin ? data.coin = item.coin : item.jackpot && (data.jackpot = item.jackpot.id);
                var sd = spinData && spinData.bonusInfo && spinData.bonusInfo.winItems;
                Array.isArray(sd) || (sd = []);
                sd.includes(item) && (data.isWinBonus = true);
              }
            }
          } catch (err) {
            _iterator.e(err);
